-- MR HASAR DANIŞMANLIK - ADK TABLOSU DÜZELTME
-- Bu SQL'i çalıştırarak eksik sütunları ekleyin
-- Tarih: 2026-01-29
...
-- Her satırı ayrı ayrı çalıştırabilirsiniz (hata verirse o sütun zaten var demektir)

ALTER TABLE adk_hesaplamalar ADD COLUMN IF NOT EXISTS hesaplama_yontemi VARCHAR(50) DEFAULT 'TBK_YARGITAY' AFTER deger_kaybi_orani;

ALTER TABLE adk_hesaplamalar ADD COLUMN IF NOT EXISTS yas_katsayi DECIMAL(5,3) DEFAULT 1.000 AFTER hesaplama_yontemi;

ALTER TABLE adk_hesaplamalar ADD COLUMN IF NOT EXISTS km_katsayi DECIMAL(5,3) DEFAULT 1.000 AFTER yas_katsayi;

ALTER TABLE adk_hesaplamalar ADD COLUMN IF NOT EXISTS hasar_katsayi DECIMAL(5,3) DEFAULT 1.000 AFTER km_katsayi;

ALTER TABLE adk_hesaplamalar ADD COLUMN IF NOT EXISTS parca_katsayi DECIMAL(5,3) DEFAULT 1.000 AFTER hasar_katsayi;

ALTER TABLE adk_hesaplamalar ADD COLUMN IF NOT EXISTS gecmis_hasar_katsayi DECIMAL(5,3) DEFAULT 1.000 AFTER parca_katsayi;

ALTER TABLE adk_hesaplamalar ADD COLUMN IF NOT EXISTS rapor_no VARCHAR(50) NULL AFTER gecmis_hasar_katsayi;

ALTER TABLE adk_hesaplamalar ADD COLUMN IF NOT EXISTS rapor_tarihi DATETIME NULL AFTER rapor_no;

ALTER TABLE adk_hesaplamalar ADD COLUMN IF NOT EXISTS created_by INT NULL AFTER rapor_tarihi;

-- Katsayılar tablosu (yoksa oluştur)
...
-- Varsayılan katsayıları ekle (IGNORE ile tekrar eklemeyi önle)
INSERT IGNORE INTO adk_katsayilar (kategori, kod, aciklama, min_deger, max_deger, katsayi) VALUES
...
INSERT IGNORE INTO adk_katsayilar (kategori, kod, aciklama, katsayi) VALUES
