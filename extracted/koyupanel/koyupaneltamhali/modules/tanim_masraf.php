<?php
/**
 * MR HASAR DANIŞMANLIK - MASRAF KALEMLERİ
...
try {
    $db = getDB();
    $masraflar = $db->query("SELECT * FROM expense_types ORDER BY name")->fetchAll();
} catch (Exception $e) {
    $error = $e->getMessage();
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    if ($_POST['action'] == 'ekle') {
        ...
        if (empty($name)) {
            $error = 'MASRAF KALEMİ ADI ZORUNLUDUR';
        } else {
            try {
                $stmt = $db->prepare("INSERT INTO expense_types (name) VALUES (?)");
                $stmt->execute([$name]);
                header("Location: tanim_masraf.php?success=1");
                exit;
            } catch (Exception $e) {
                $error = $e->getMessage();
            }
    ...
    if ($_POST['action'] == 'sil') {
        ...
        if ($id) {
            try {
                $stmt = $db->prepare("DELETE FROM expense_types WHERE id = ?");
                $stmt->execute([$id]);
                header("Location: tanim_masraf.php?success=1");
                exit;
            } catch (Exception $e) {
    ...
    if ($_POST['action'] == 'guncelle') {
        ...
        if ($id && !empty($name)) {
            try {
                $stmt = $db->prepare("UPDATE expense_types SET name = ? WHERE id = ?");
...
<div class="page-title">
    ...
    <button onclick="document.getElementById('ekleModal').style.display='flex'" class="btn btn-suc" style="margin-left:auto;font-size:12px">
        <i class="fas fa-plus"></i> YENİ MASRAF KALEMİ
    </button>
</div>
...
<div id="ekleModal" style="display:none;position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,0.8);z-index:99999;align-items:center;justify-content:center;padding:20px">
    <div style="background:#1e293b;border-radius:12px;width:100%;max-width:400px;border:2px solid #3b82f6">
        <div style="padding:20px;border-bottom:1px solid #334155;display:flex;justify-content:space-between;align-items:center">
            <h3 style="margin:0;color:#f1f5f9;font-size:16px"><i class="fas fa-plus-circle"></i> YENİ MASRAF KALEMİ</h3>
        ...
        <form method="POST" style="padding:20px">
            ...
            <div class="frm-grp" style="margin-bottom:15px">
                <label class="frm-lbl">MASRAF KALEMİ ADI <span class="req">*</span></label>
...
<div class="panel">
    <div class="panel-head">
        <div class="panel-title"><i class="fas fa-list"></i> MASRAF KALEMLERİ (<?= count($masraflar) ?>)</div>
    </div>
    <div class="tbl-wrap">
        <table>
            <thead>
                <tr>
                    <th>ID</th>
                    <th>MASRAF KALEMİ ADI</th>
                    <th>İŞLEM</th>
                </tr>
            </thead>
            <tbody>
                <?php if (empty($masraflar)): ?>
                <tr><td colspan="3" style="text-align:center;padding:30px;color:var(--text2)">KAYIT BULUNAMADI</td></tr>
                ...
                <tr>
                    ...
                    <td>
                        <div class="icons">
                            <button onclick="duzenle(<?= $m['id'] ?>, '<?= e($m['name']) ?>')" class="ic edit" style="border:none;cursor:pointer"><i class="fas fa-edit"></i></button>
