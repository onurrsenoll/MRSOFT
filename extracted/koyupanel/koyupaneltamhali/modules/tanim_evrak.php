<?php
/**
 * MR HASAR DANIŞMANLIK - EVRAK TÜRLERİ
...
try {
    $db = getDB();
    $evraklar = $db->query("SELECT * FROM document_types ORDER BY name")->fetchAll();
} catch (Exception $e) {
    $error = $e->getMessage();
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    if ($_POST['action'] == 'ekle') {
        ...
        if (empty($name)) {
            $error = 'EVRAK TÜRÜ ADI ZORUNLUDUR';
        } else {
            try {
                $stmt = $db->prepare("INSERT INTO document_types (name) VALUES (?)");
                $stmt->execute([$name]);
                header("Location: tanim_evrak.php?success=1");
                exit;
            } catch (Exception $e) {
                $error = $e->getMessage();
            }
...
<div class="page-title">
    ...
    <button onclick="document.getElementById('ekleModal').style.display='flex'" class="btn btn-suc" style="margin-left:auto;font-size:12px">
        <i class="fas fa-plus"></i> YENİ EVRAK TÜRÜ
    </button>
</div>
...
<div id="ekleModal" style="display:none;position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,0.8);z-index:99999;align-items:center;justify-content:center;padding:20px">
    <div style="background:#1e293b;border-radius:12px;width:100%;max-width:400px;border:2px solid #3b82f6">
        <div style="padding:20px;border-bottom:1px solid #334155;display:flex;justify-content:space-between;align-items:center">
            <h3 style="margin:0;color:#f1f5f9;font-size:16px"><i class="fas fa-plus-circle"></i> YENİ EVRAK TÜRÜ</h3>
        ...
        <form method="POST" style="padding:20px">
            ...
            <div class="frm-grp" style="margin-bottom:15px">
                <label class="frm-lbl">EVRAK TÜRÜ ADI <span class="req">*</span></label>
...
<div class="panel">
    <div class="panel-head">
        <div class="panel-title"><i class="fas fa-list"></i> EVRAK TÜRLERİ (<?= count($evraklar) ?>)</div>
    </div>
    <div class="tbl-wrap">
        <table>
            <thead>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        <div class="icons">
                            <button onclick="duzenle(<?= $e['id'] ?>, '<?= e($e['name']) ?>')" class="ic edit" style="border:none;cursor:pointer"><i class="fas fa-edit"></i></button>
